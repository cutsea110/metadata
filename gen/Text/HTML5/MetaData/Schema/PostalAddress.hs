{-# LANGUAGE OverloadedStrings #-}
{-# LANGUAGE DeriveDataTypeable #-}
module Text.HTML5.MetaData.Schema.PostalAddress where

--  Valid: 2014-04-03 ( Schema.rdfs.org )

import Text.HTML5.MetaData.Class
import Text.HTML5.MetaData.Type
import Data.Text
import Data.Typeable
import {-# SOURCE #-} qualified Text.HTML5.MetaData.Schema.Thing
import {-# SOURCE #-} qualified Text.HTML5.MetaData.Schema.Intangible
import {-# SOURCE #-} qualified Text.HTML5.MetaData.Schema.StructuredValue
import {-# SOURCE #-} qualified Text.HTML5.MetaData.Schema.ContactPoint

-- | 
--
--   [@id@] PostalAddress
--
--   [@label@] Postal Address
--
--   [@comment@] 
--
--   [@ancestors@] @'Thing','Intangible','StructuredValue','ContactPoint'@
--
--   [@subtypes@]
--
--   [@supertypes@] @'ContactPoint'@
--
--   [@url@] <http://schema.org/PostalAddress>
data PostalAddress = PostalAddress { additionalType :: AdditionalType
                                   , alternateName :: AlternateName
                                   , description :: Description
                                   , image :: Image
                                   , name :: Name
                                   , sameAs :: SameAs
                                   , url :: Url
                                   , areaServed :: AreaServed
                                   , availableLanguage :: AvailableLanguage
                                   , contactOption :: ContactOption
                                   , contactType :: ContactType
                                   , email :: Email
                                   , faxNumber :: FaxNumber
                                   , hoursAvailable :: HoursAvailable
                                   , productSupported :: ProductSupported
                                   , telephone :: Telephone
                                   , addressCountry :: AddressCountry
                                   , addressLocality :: AddressLocality
                                   , addressRegion :: AddressRegion
                                   , postalCode :: PostalCode
                                   , postOfficeBoxNumber :: PostOfficeBoxNumber
                                   , streetAddress :: StreetAddress
                                   }
                     deriving (Show, Read, Eq, Typeable)

instance MetaData PostalAddress where
  _label         = const "Postal Address"
  _comment_plain = const ""
  _comment       = const ""
  _url           = const "http://schema.org/PostalAddress"
  _ancestors     = const [typeOf (undefined :: Text.HTML5.MetaData.Schema.Thing.Thing), typeOf (undefined :: Text.HTML5.MetaData.Schema.Intangible.Intangible), typeOf (undefined :: Text.HTML5.MetaData.Schema.StructuredValue.StructuredValue), typeOf (undefined :: Text.HTML5.MetaData.Schema.ContactPoint.ContactPoint)]
  _subtypes      = const []
  _supertypes    = const [typeOf (undefined :: Text.HTML5.MetaData.Schema.ContactPoint.ContactPoint)]